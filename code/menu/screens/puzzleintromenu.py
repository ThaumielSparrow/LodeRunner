...
class PuzzleIntroMenu(Menu):
    ...
    def handle_build_event(self, event, control_center, universe):
        ...
        # Calculate puzzle intro tooltip text.
        tooltip_text = control_center.get_localization_controller().get_label("puzzle-default-intro:message") # Default

        # If the map has a "tooltip" param, then we'll use that tooltip text instead of the default.
        if ( universe.get_active_map().get_param("tooltip") ):

            # Prepend text
            tooltip_text = "%s" % universe.get_active_map().get_param("tooltip")


        # Fetch the puzzle intro template
        template = self.fetch_xml_template("puzzle.menu.intro").add_parameters({
            ...
            "@tooltip-text": xml_encode( "%s" % localization_controller.translate( tooltip_text ) ),
